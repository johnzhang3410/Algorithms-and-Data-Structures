...
void Resizable_deque<Type>::push_front( Type const &obj ) {
	...
	if (size() == capacity()){
		...
		//Moving everything from ifront to the end of the array to the back
		for (int i{array_capacity - 1}; i != array_capacity - new_size - 1; i--){
		...
		ifront = array_capacity - new_size;
...
void Resizable_deque<Type>::push_back( Type const &obj ) {
	...
	//When the array is full, double the capacity and copy everything after ifront to the end of the expanded array
	//We can now continue pushing onto the back without worring about loss of data
	if (size() == capacity()){
